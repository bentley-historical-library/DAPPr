...
In DSpace, Communities, Collections, and Items typically get minted a Handle Identifier. You can reference these objects in the REST API by their handle, as opposed to having to use the internal item-ID.
  ...
  * `bitstreams = deepblue.get_bitstreams()`: Return all bitstreams in DSpace.
  * `bitstream = deepblue.get_bitstream(Bitstream ID INTEGER)`: Return bitstream.
  * `policy = deepblue.get_bitstream_policy(Bitstream ID INTEGER)`: Return bitstream policies.
  ...
  * `deepblue.delete_bitstream(Bitstream ID)`: Delete bitstream from DSpace.
  * `deepblue.delete_bitstream_policy(Bitstream ID INTEGER, Policy ID INTEGER)`: Delete bitstream policy.
